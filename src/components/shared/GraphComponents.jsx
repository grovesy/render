// src/components/shared/GraphComponents.jsx
import React from "react";
import { Handle, Position } from "@xyflow/react";

export function UMLNode({ data }) {
  const { title, attrs, collapsed, onToggleCollapse } = data;
  const rowH = 24;
  const headerH = 44;
  const padding = 12;
  const maxField = Math.max(...attrs.map(a => (a.field || "").length), 0);
  const charW = 8;
  const sepX = padding + maxField * charW + 12;
  const width = Math.max(240, sepX + 120 + padding);
  const expandedHeight = headerH + padding + attrs.length * rowH + padding;
  const height = collapsed ? headerH : expandedHeight;

  return (
    <div style={{ width, height, position: 'relative' }}>
      {/* Connection handles - multiple positions for smart routing */}
      <Handle type="target" position={Position.Left} id="left" style={{ opacity: 0, left: 0, top: '50%' }} />
      <Handle type="target" position={Position.Right} id="right" style={{ opacity: 0, right: 0, top: '50%' }} />
      <Handle type="source" position={Position.Left} id="left" style={{ opacity: 0, left: 0, top: '50%' }} />
      <Handle type="source" position={Position.Right} id="right" style={{ opacity: 0, right: 0, top: '50%' }} />
      
      <svg width={width} height={height} className="uml-node">
        <rect className="uml-node__body" x={0} y={0} width={width} height={height} rx={8} ry={8} />
        <rect className="uml-node__header" x={0} y={0} width={width} height={headerH} rx={8} ry={8} />
        <text className="uml-node__title" x={width/2} y={headerH/2+6} textAnchor="middle">{title}</text>
        
        {/* Collapse/Expand toggle icon */}
        <g 
          onClick={(e) => {
            e.stopPropagation();
            if (onToggleCollapse) onToggleCollapse();
          }}
          style={{ cursor: 'pointer' }}
        >
          <rect className="uml-node__toggle" x={width - 32} y={8} width={24} height={28} rx={4} />
          <text className="uml-node__toggle-icon" x={width - 20} y={28} textAnchor="middle">
            {collapsed ? '▼' : '▲'}
          </text>
        </g>
        
        {!collapsed && (
          <>
            {/* vertical separator */}
            <line className="uml-node__separator" x1={sepX} y1={headerH} x2={sepX} y2={height} />
            {/* rows */}
            {attrs.map((a, i) => {
              const y = headerH + padding + i * rowH;
              return (
                <g key={i}>
                  <text className="uml-node__field" x={padding} y={y+14}>{a.field}</text>
                  <text className="uml-node__type" x={sepX+12} y={y+14}>{a.type}</text>
                  <line className="uml-node__divider" x1={padding} y1={y+rowH-4} x2={width-padding} y2={y+rowH-4} />
                </g>
              );
            })}
          </>
        )}
      </svg>
    </div>
  );
}

export function DomainBox({ data }) {
  const { label, width, height } = data;
  return (
    <div style={{ width, height, position: 'relative', pointerEvents: 'none' }}>
      <svg width={width} height={height} className="domain-box">
        <rect 
          className="domain-box__container"
          x={0} 
          y={0} 
          width={width} 
          height={height} 
          rx={12} 
          ry={12}
          strokeDasharray="8,4"
        />
        <text 
          className="domain-box__label-text"
          x={width/2} 
          y={25} 
          textAnchor="middle"
        >
          {label}
        </text>
      </svg>
    </div>
  );
}

export const nodeTypes = { uml: UMLNode, domainBox: DomainBox };
