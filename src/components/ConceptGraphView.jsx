...
// API base (frontend .env: VITE_API_BASE_URL=http://localhost:3000)
      ...
      try {
        ...
        (graphData.entities || []).forEach((e) => {
          ...
          e.refs.forEach((r) => {
            ...
            const key = `${refDomain}:${refModel}`;
            const tgt = entityCells[key];
            if (!tgt) {
              console.warn("[GraphView] No target cell for", key, "from", r.ref);
              return;
            }
        ...
        // ---- DirectedGraph layout + domain offset ----
