...
Frontend: React + Vite in `src/` (components: `ModelGraphView.jsx`, `ConceptGraphView.jsx`, `FileViewer.jsx`, `LeftSidebar.jsx`). `public/app.js` contains the original non-React example.
...
- When changing parsing or ID handling update `schemaGraph.js` and verify `ModelGraphView.jsx` and `public/app.js` remain compatible.
...
Files to read first
- `server.js`, `schemaGraph.js`, `conceptGraph.js`, `src/App.jsx`, `src/components/ModelGraphView.jsx`, `src/components/ConceptGraphView.jsx` and `examples/*`.
